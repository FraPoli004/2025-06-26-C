...
class Controller:
    ...
    def handleBuildGraph(self, e):
        ...
        if  ai is None or af is None:  # <-- ADATTA nome dropdown; valida TUTTI gli input
            ...
            self._view._txt_result.controls.append(ft.Text("Seleziona un valore da entrambe i menù a tendina.", color="red"))
        ...
        if af < ai:
            self._view._txt_result.controls.clear()
            self._view._txt_result.controls.append(ft.Text("seleziona un anno iniziale minore di quello finale", color="red"))
            self._view.update_page()
            return
    ...
    def handlePrintDetails(self, e):
        ...
        self._view._txt_result.controls.append(ft.Text(f"Nel grafo sono presenti {num} componenti connesse, la più lunga ha {len(massima)} nodi"))
